...
fun SimpleTopBar(title: String, navController: NavController) {
    TopAppBar(
        title = { Text(title, color = CryptoWhite, fontWeight = FontWeight.Bold) },
        navigationIcon = {
            IconButton(
                onClick = {
                ...
                Icon(
                    ...
                    contentDescription = "Back", tint = CryptoWhite,
                    modifier = Modifier.size(24.dp)
                )
            }
        },
        colors = TopAppBarDefaults.topAppBarColors(containerColor = CryptoBlackBackground)
    )
}